...
| ドキュメントのメタデータ | external versionを使ったdelete検査と削除tombstoneは未対応です。初回`_seq_no`、レプリカ数を含む書き込み応答の`_shards.total`、512 byteの`_id`上限、DELETEでの`if_seq_no` / `if_primary_term`ペア必須検査は回帰テストがあります。 |
