...
summary:
  sections:
    ...
    - Painless (read-only, github.com/shibukawa/painlessscript-go v0.2.0, extension registry available): script_fields, script/script_score queries, function_score's script_score function, sort by _script, bucket_script/bucket_selector pipelines; document scripts read doc[field].value/.size(), params, _score, bucket scripts read only params (own static params merged with buckets_path values); scripts use Math/DateTime/Instant/String/local list+map; compile or runtime failure reports script_exception
    - approximations: BM25 scores, bleve analyzers for indexing, exact cardinality, t-digest percentile edge cases, numeric range/sort precision, text fielddata aggregation, ignored _doc_count, ignored search options (timeout, profile timings, stats, runtime_mappings), phrase suggester scoring (stupid-backoff n-gram, not Lucene's smoothed LM), error_trace stack_trace (one fabricated frame naming the exception, on every error/root_cause/caused_by, not a real Java trace)
    ...
    - unsupported (400): mutating/aggregating Painless scripts (ctx._source updates, update_by_query/reindex with a script, scripted_metric), object/array script params, script instead of field in bucket/value-source aggregations, moving_fn (needs a values array script param the runtime can't construct yet), script_heuristic, kNN, most span queries (span_or/first/not/containing/within, field_masking_span), data streams, ingest pipelines, rollover/shrink/split, _tasks, node statistics, YAML/CBOR/SMILE bodies, ...
