...
summary:
  ...
  packaging:
    ...
    pypi: project osmem-server (PyPI name osmem is taken by an unrelated tool), import osmem_server; one wheel per platform tag containing the binary
    maven: groupId jp.shibu.osmem (namespace jp.shibu, DNS TXT on shibu.jp; decided 2026-09-12), Java package jp.shibu.osmem, artifacts osmem + osmem-server-binaries with os-arch classifier jars; multi-module pom with release profile (sources, javadoc, gpg, central-publishing-maven-plugin 0.11.0)
