...
- **効果のない検索オプション。** `timeout`と`stats`は検証しますが効果はありません。`profile`はタイミングが0のOpenSearchと同じ構造を返し、名前付きの`search_pipeline`は存在している必要があります。`runtime_mappings`は拒否します。
