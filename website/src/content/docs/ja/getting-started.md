...
この設計が限界を決めます。普通のクエリなら順位はOpenSearchと一致しますが、`_score`の数値は異なるので、テストでスコアの値を検証してはいけません。Painlessスクリプトは未対応で400を返します。利用する機能の[互換性](../compatibility/)を事前に確認してください。受理されてもOpenSearchと異なる動作をするオプションもあります。
