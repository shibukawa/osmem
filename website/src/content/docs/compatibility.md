...
| Document metadata | External-version delete checks and deletion tombstones are ignored. Initial `_seq_no`, configured replica counts in write `_shards.total`, the 512-byte `_id` limit, and DELETE's paired `if_seq_no` / `if_primary_term` requirement have regression coverage. |
