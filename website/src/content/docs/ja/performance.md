...
2026-09-13、Apple M3、macOS arm64、Go 1.27.0、Python 3.14.7、Node.js 26.8.1、Java 25.0.2、OrbStack上のDocker 29.4.0で計測しました。osmemの全経路で同じ525-byte seed（2 index、3 document）を読み込みます。日本語有効ではseed内のkuromoji analyzerを初期化し、無効ではCJK fallbackを使います。
...
| Testcontainers Go + OpenSearch | — | **6.26秒** | 5.79〜6.89秒 (n=5) | Testcontainers-Go 0.44.0。毎回fresh container、image取得済み。初回Ryuk起動を含む |
...
| Testcontainers Go + OpenSearch | container 952.9MiB + runner RSS増分4.5MiB = **957.5MiB** | — | Go runnerは起動前15.3MiB、ready時19.9MiB。Docker daemonとRyukは除外 |
...
Testcontainersはテスト側からcontainer runtimeを扱うwrapperで、検索engineではありません。JVMの大きく変動するbaselineをcontainer消費と誤認しないよう、Go版(`testcontainers-go` 0.44.0)で計測しました。fresh containerを5回起動し、index準備まで平均6.26秒です。Go test processはcontainer起動直前にRSS平均15.3MiB、ready時に19.9MiBで、差分4.5MiBだけをcontainerの952.9MiBに加算しました。containerとrunner増分の合計は957.5MiBです。初回はTestcontainersのRyuk helperも起動しましたが、memoryにはhelperとDocker daemonを含めません。class全体でcontainerを共有すれば、起動時間はそのclass内のtestで按分されます。
