...
この設計が限界を決めます。普通のクエリなら順位はOpenSearchと一致しますが、`_score`の数値は異なるので、テストでスコアの値を検証してはいけません。Painlessスクリプトは読み取り用途(`script_fields`、`script`/`script_score`クエリ、`_script`によるソート)では動きますが、文書を書き換えるスクリプトは400を返します。利用する機能の[互換性](../compatibility/)を事前に確認してください。受理されてもOpenSearchと異なる動作をするオプションもあります。
